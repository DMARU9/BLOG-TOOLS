...
DUCKDUCKGO_SEARCH_DESCRIPTION = (
    "A free search engine (DuckDuckGo) for web searches. "
    "Useful for when you need to answer questions about current events without requiring an API key."
)
...
async def duckduckgo_search(
    ...
    """Execute DuckDuckGo search queries and return combined results.
    
    Args:
        queries: List of search query strings to execute
        max_results: Maximum number of results per query
        config: Runtime configuration (not used but kept for compatibility)
...
async def get_search_tool(search_api: SearchAPI):
    ...
    if search_api == SearchAPI.ANTHROPIC:
        ...
        # Configure DuckDuckGo search tool (free, no API key required)
        duckduckgo_tool = StructuredTool.from_function(
            coroutine=duckduckgo_search,
            name="web_search",
            description=DUCKDUCKGO_SEARCH_DESCRIPTION,
            args_schema=None  # Use default args
        )
