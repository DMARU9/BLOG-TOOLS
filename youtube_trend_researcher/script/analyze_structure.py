#!/usr/bin/env python3
"""Analyze the structure of youtube_trend_researcher and emit diagrams + metrics.

Outputs (under <repo>/artifacts):
  - pyreverse/classes_ytr.{dot,png,svg}   : UML class diagram (attributes + associations)
  - pyreverse/packages_ytr.{dot,png,svg} : module import dependency graph
  - pydeps_ytr.svg                        : clustered dependency graph (pydeps)
...
def pyreverse(src: Path, out: Path) -> None:
    out.mkdir(parents=True, exist_ok=True)
    # -o dot emits .dot files in cwd; run inside out/ then move up if needed.
    run(["pyreverse", "-o", "dot", "-p", "ytr", str(src)], cwd=out, check=False)
    for kind in ("classes", "packages"):
        dot = out / f"{kind}_ytr.dot"
        if not dot.exists():
            continue
        run(["dot", "-Tpng", str(dot), "-o", str(out / f"{kind}_ytr.png")])
        run(["dot", "-Tsvg", str(dot), "-o", str(out / f"{kind}_ytr.svg")])


def pydeps(src: Path, out: Path) -> None:
    ...
    svg = out / "pydeps_ytr.svg"
    png = out / "pydeps_ytr.png"
...
def find_isolated(src: Path, out: Path) -> list[str]:
...
def main() -> None:
    ...
    ap.add_argument("--src", type=Path, default=REPO_ROOT / "src" / "youtube_trend_researcher")
    ...
    pyreverse(args.src, pyreverse_out)
    ...
    isolated = find_isolated(args.src, out)
